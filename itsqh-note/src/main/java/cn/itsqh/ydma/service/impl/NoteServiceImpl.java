...
public class NoteServiceImpl implements NoteService {
	...
	@Override
	public YdmaResult addLikeCountById(int id) {
		YdmaResult result = new YdmaResult();
		int ia = noteDao.updateByLikeCount(id);
		if(ia > 0) {
			result.setCode(YdmaConstant.SUCCESS);
			result.setMsg(YdmaConstant.UPDATE_NOTELIKECOUNT_SUCCESS_MSG);
			return result;
		}
		result.setCode(YdmaConstant.ERROR1);
		result.setMsg(YdmaConstant.UPDATE_NOTELIKECOUNT_ERROR2_MSG);
		return result;
	}

	@Override
	public YdmaResult selectByUserIdAndVideoId2(int userId, int voidId) {
		YdmaResult result = new YdmaResult();
		List<Note> list =  noteDao.selectByUserIdAndVideoId(userId,voidId);
		if(list != null) {
			result.setCode(YdmaConstant.SUCCESS);
			result.setMsg(YdmaConstant.SELECT_SUCCESS_MSG);
			result.setData(list);
	...
	@Override
	public YdmaResult findById(int noteId) {
		YdmaResult result = new YdmaResult();
