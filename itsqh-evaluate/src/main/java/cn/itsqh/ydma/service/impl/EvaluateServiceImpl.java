...
public class EvaluateServiceImpl implements EvaluateService {
	...
	@Override
	public YdmaResult addEvaluate(String token,int videoid,String context) {
	...
	public YdmaResult findByVideoId(int id,int page,int size) {
		...
		if(list.size()>0) {
			for(Evaluate evaluate:list) {
				int userId = evaluate.getUseId();
				String url = "http://localhost:7001/user/findUser?id="+userId;
				YdmaResult userResult =restemplate.getForObject(url, YdmaResult.class);
				if(userResult.getCode()==YdmaConstant.SUCCESS) {
					//System.out.println(courseResult.getData());
					Object obj =  userResult.getData();
					evaluate.setUser(obj);
				}
			}
			result.setCode(YdmaConstant.SUCCESS );
